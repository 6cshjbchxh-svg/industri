...
function downloadSvg() {
  ...
  link.href = url;
  link.download = 'arbeidstegning.svg';
...
function getMailSubjectAndBody() {
  const date = new Date().toISOString().slice(0, 10);
  const subject = `Arbeidstegning ${date}`;
  const body =
    `Hei,\n\nVedlagt ligger arbeidstegning og skisse eksportert fra appen.\n` +
    `Filene inneholder 2D/3D-visning og mål.\n\nMvh`;
  return { subject, body };
}
...
function buildPdfSheetSvg() {
  ...
  const grad = makeSvgElement('linearGradient', { id: 'steelGrad', x1: '0%', y1: '0%', x2: '100%', y2: '100%' });
  grad.appendChild(makeSvgElement('stop', { offset: '0%', 'stop-color': '#d1d5db' }));
  grad.appendChild(makeSvgElement('stop', { offset: '50%', 'stop-color': '#6b7280' }));
  grad.appendChild(makeSvgElement('stop', { offset: '100%', 'stop-color': '#e5e7eb' }));
  defs.appendChild(grad);
  exportSvg.appendChild(defs);

  drawRect(exportSvg, 20, 20, sheetW - 40, sheetH - 40, { stroke: '#111827', fill: '#f8fafc', 'stroke-width': 1.8 });
  drawText(exportSvg, 60, 90, 'JSO', { 'font-size': 74, 'font-weight': 800, fill: '#0d4a7f' });
  drawText(exportSvg, 185, 74, 'INDUSTRI', { 'font-size': 34, 'font-weight': 700, fill: '#555' });
  drawText(exportSvg, 185, 108, 'TEGNING', { 'font-size': 34, 'font-weight': 700, fill: '#555' });

  const main3d = { x: 60, y: 130, w: 1080, h: 520 };
  const top2d = { x: 60, y: 700, w: 690, h: 300 };
  const cut2d = { x: 780, y: 700, w: 360, h: 300 };
  const info = { x: 1165, y: 130, w: 380, h: 870 };

  for (const b of [main3d, top2d, cut2d, info]) {
    drawRect(exportSvg, b.x, b.y, b.w, b.h, { stroke: '#111827', fill: '#ffffff', 'stroke-width': 1.2 });
  }
  drawText(exportSvg, main3d.x + 14, main3d.y + 30, '3D-DETALJ', { 'font-size': 34, 'font-weight': 700 });
  drawText(exportSvg, top2d.x + 14, top2d.y + 30, 'TOPPVISNING 2D', { 'font-size': 28, 'font-weight': 700 });
  drawText(exportSvg, cut2d.x + 14, cut2d.y + 30, 'SNITT / MÅL', { 'font-size': 28, 'font-weight': 700 });

  const b3 = combineBounds(sketchShapes, true);
  const b2 = combineBounds(sketchShapes, false);
  const scale3 = Math.min((main3d.w - 70) / Math.max(1, b3.maxX - b3.minX), (main3d.h - 90) / Math.max(1, b3.maxY - b3.minY));
  const scale2 = Math.min((top2d.w - 70) / Math.max(1, b2.maxX - b2.minX), (top2d.h - 80) / Math.max(1, b2.maxY - b2.minY));
  const tx3 = main3d.x + 35 - b3.minX * scale3;
  const ty3 = main3d.y + 55 - b3.minY * scale3;
  const tx2 = top2d.x + 35 - b2.minX * scale2;
  const ty2 = top2d.y + 55 - b2.minY * scale2;
  const txCut = cut2d.x + 35 - b2.minX * scale2 * 0.8;
  const tyCut = cut2d.y + 70 - b2.minY * scale2 * 0.8;
  ...
  for (const shape of sketchShapes) {
    ...
    drawExportShape(g2, shape, tx2, ty2, scale2, false, '#111827');
    drawExportShape(gCut, shape, txCut, tyCut, scale2 * 0.8, false, '#111827');
  }

  const tableTop = info.y + 60;
  drawText(exportSvg, info.x + 20, info.y + 34, 'PRODUKT: FRI TEGNING', { 'font-size': 22, 'font-weight': 700 });
  drawLine(exportSvg, info.x, tableTop, info.x + info.w, tableTop, { stroke: '#111827', 'stroke-width': 1.1 });
  drawText(exportSvg, info.x + 20, tableTop + 30, 'Figur', { 'font-size': 16, 'font-weight': 700 });
  drawText(exportSvg, info.x + 100, tableTop + 30, 'Mål', { 'font-size': 16, 'font-weight': 700 });
  drawText(exportSvg, info.x + 320, tableTop + 30, 'Mat', { 'font-size': 16, 'font-weight': 700 });
  let y = tableTop + 50;
  for (const shape of sketchShapes) {
    drawLine(exportSvg, info.x + 10, y - 14, info.x + info.w - 10, y - 14, { stroke: '#cbd5e1', 'stroke-width': 1 });
    drawText(exportSvg, info.x + 20, y, `${shape.label} (${shape.type})`, { 'font-size': 14 });
    drawText(exportSvg, info.x + 100, y, shapeMeasureText(shape), { 'font-size': 13 });
    drawText(exportSvg, info.x + 320, y, shape.material || 'stal', { 'font-size': 13 });
    y += 30;
  }
  drawText(exportSvg, info.x + 20, info.y + info.h - 50, `Dato: ${new Date().toISOString().slice(0, 10)}`, { 'font-size': 14 });
  drawText(exportSvg, info.x + 220, info.y + info.h - 50, 'Enhet: mm', { 'font-size': 14 });
  drawText(exportSvg, info.x + 20, info.y + info.h - 24, 'Målestokk: Auto', { 'font-size': 14 });
  drawText(exportSvg, info.x + 220, info.y + info.h - 24, 'Tegning: Arbeidsark', { 'font-size': 14 });
...
async function sendDrawingByEmail() {
  try {
    const date = new Date().toISOString().slice(0, 10);
    const sheetSvg = buildPdfSheetSvg();
    const sheetMarkup = new XMLSerializer().serializeToString(sheetSvg);
    const sketchMarkup = new XMLSerializer().serializeToString(svg);
    const sheetFile = new File([sheetMarkup], `arbeidstegning-${date}.svg`, { type: 'image/svg+xml' });
    const sketchFile = new File([sketchMarkup], `skisse-${date}.svg`, { type: 'image/svg+xml' });
    const { subject, body } = getMailSubjectAndBody();

    if (navigator.share && navigator.canShare) {
      const shareData = { title: subject, text: body, files: [sheetFile, sketchFile] };
      if (navigator.canShare(shareData)) {
        await navigator.share(shareData);
        statusEl.textContent = 'Tegning sendt/delt.';
        return;
      }
    }

    triggerFileDownload(sheetFile, sheetFile.name);
    triggerFileDownload(sketchFile, sketchFile.name);
    const fallbackBody = `${body}\n\nFilene ble lastet ned automatisk. Legg dem ved i e-posten.`;
    window.location.href = `mailto:?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(fallbackBody)}`;
    statusEl.textContent = 'E-postutkast åpnet. Legg ved de nedlastede filene.';
  } catch (error) {
    statusEl.textContent = 'Klarte ikke sende e-post. Prøv igjen.';
  }
}
...
function setActiveTool(tool) {
  activeTool = tool;
  if (tool !== 'measure') measureStartRef = null;
  ...
  if (tool !== 'move') selectedShapeIndex = -1;
  renderSketchShapes();
}
...
function buildPreviewShape(start, current) {
  ...
  if (activeTool === 'line') {
    const snapped = getSnappedPoint(start, current);
...
function onPointerDown(event) {
  ...
  if (activeTool === 'measure') {
    ...
    if (!measureStartRef) {
      ...
      sketchMeasures.push({ startRef: measureStartRef, endRef: ref });
...
function addShapeFromDrag(start, end) {
  ...
  if (activeTool === 'line') {
    const snappedEnd = getSnappedPoint(start, end);
    sketchShapes.push({
      type: 'line',
      material,
      depth: 0,
  ...
  if (activeTool === 'rect') {
    ...
    if (width > 2 && height > 2) sketchShapes.push({ type: 'rect', material, depth: 0, label: getNextShapeLabel(), x, y, width, height });
    return;
  }
  ...
  if (r > 2) sketchShapes.push({ type: 'circle', material, depth: 0, label: getNextShapeLabel(), cx: start.x, cy: start.y, r });
}
...
function onPointerUp(event) {
  ...
  isDrawing = false;
  const endPoint = pointerToSvgPoint(event);
  if (previewElement && sketchGroup && sketchGroup.contains(previewElement)) {
    sketchGroup.removeChild(previewElement);
    previewElement = null;
  }
...
blankSheetBtn.addEventListener('click', toggleBlankSheet);
